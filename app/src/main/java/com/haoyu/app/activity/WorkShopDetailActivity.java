...
public class WorkShopDetailActivity extends BaseActivity {
    ...
    private void updateBriefList(List<BriefingEntity> mDatas, Paginator paginator) {
        ...
        briefAdapter.setItemClickCallBack(new BriefingAdapter.onItemClickCallBack() {
            @Override
        ...
        briefAdapter.setDisposeCallBack(new BriefingAdapter.onDisposeCallBack() {
            @Override
    ...
    /*获取优秀学员*/
    private void getExcellentUsers() {  //数据为测试用
        String usersUrl = Constants.OUTRT_NET + "/m/workshop_user/" + workshopId + "/excellent_users?page=" + userPage + "&limit=" + limit;
        OkHttpClientManager.getAsyn(context, usersUrl, new OkHttpClientManager.ResultCallback<WorkShopExecllentUserResult>() {
            @Override
            public void onError(Request request, Exception e) {
