...
//                    bitmap.recycle();
     ...
     * 将图片保存在本地
     */
    private void saveOutput(final Bitmap croppedImage, final Bitmap.CompressFormat outputFormat, final File saveFile) {
        final PublicTipDialog dialog = new PublicTipDialog(getContext());
        dialog.show();
        Flowable.fromCallable(new Callable<File>() {
            @Override
            public File call() throws Exception {
                OutputStream outputStream = getContext().getContentResolver().openOutputStream(Uri.parse("file://" + saveFile));
