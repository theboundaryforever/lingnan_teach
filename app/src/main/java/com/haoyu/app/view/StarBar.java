...
public class StarBar extends View {
	...
	private float starMark = 0.00F; // 评分星星
	...
	private void init(Context context, AttributeSet attrs) {
		...
		this.starDistance = (int) mTypedArray.getDimension(
				R.styleable.RatingBar_starDistance, 0);
		this.starSize = (int) mTypedArray.getDimension(
				R.styleable.RatingBar_starSize, 20);
		this.starCount = mTypedArray.getInteger(
				R.styleable.RatingBar_starNumber, 5);
		this.starEmptyDrawable = mTypedArray
				.getDrawable(R.styleable.RatingBar_starEmpty);
		this.starFillBitmap = drawableToBitmap(mTypedArray
				.getDrawable(R.styleable.RatingBar_starFill));
		this.canEdit = mTypedArray.getBoolean(R.styleable.RatingBar_isEdit,
				true);
		mTypedArray.recycle();

		paint = new Paint();
		paint.setAntiAlias(true);
		paint.setShader(new BitmapShader(starFillBitmap,
	...
	public void setStarMark(float mark) {
		if (integerMark) {
			...
			starMark = Math.round(mark * 100) * 1.0f / 100;
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(starSize * starCount + starDistance
				* (starCount - 1), starSize);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < starCount; i++) {
			starEmptyDrawable.setBounds((starDistance + starSize) * i, 0,
		...
		if (starMark > 1) {
			...
			if (starMark - (int) (starMark) == 0) {
				...
				canvas.drawRect(
						...
						0,
						starSize
								* (Math.round((starMark - (int) (starMark)) * 10) * 1.0f / 10),
						starSize, paint);
			}
		} else {
	...
	private Bitmap drawableToBitmap(Drawable drawable) {
		if (drawable == null)
			return null;
		Bitmap bitmap = Bitmap.createBitmap(starSize, starSize,
