...
/**
 * 创建日期：2017/7/11 on 13:33
 * 描述:
 * 作者:马飞奔 Administrator
 */
public class AppCheckBox extends View implements Checkable {
    ...
    public AppCheckBox(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.AppCheckBox);
        int tickColor = ta.getColor(R.styleable.AppCheckBox_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.AppCheckBox_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.AppCheckBox_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.AppCheckBox_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.AppCheckBox_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.AppCheckBox_stroke_width, PixelFormat.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mListener != null) {
            mListener.onCheckedChanged(AppCheckBox.this, mChecked);
        }
    }
    ...
    private int measureSize(int measureSpec) {
        int defSize = PixelFormat.dp2px(getContext(), DEF_DRAW_SIZE);
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(AppCheckBox checkBox, boolean isChecked);
    }
}
